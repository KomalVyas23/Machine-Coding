window.addEventListener("load", function () {
    ...
    for (let ri = 0; ri < nr; ri++) {
        ...
        for (let ci = 0; ci < nc; ci++) {
            ...
            //Creating a span for keeping bishop
            let span = document.createElement("span");
            span.classList.add("chessPiece");
            span.setAttribute("data-index", `${ri}-${ci}`);
            td.appendChild(span);
    ...
    table.addEventListener("mouseover", function (e) {
        ...
        let selectedSpan = e.target.querySelector('span');
        selectedSpan.classList.add('current');
    ...
    table.addEventListener("mouseleave", function (e) {
        allBoxes.forEach(box => {
            box.firstChild.classList.remove("current");
        })
    })
